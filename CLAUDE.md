...
Version: 0.8.1 | Plugins: 4 | Last Updated: 2026-07-24
...
Structure: Per-plugin manifests (`plugins/<plugin>/.claude-plugin/plugin.json`); skills carry no manifest of their own (SKILL.md frontmatter only)
...
| `shared/scripts/sap_check_gui_login_status.vbs` | sap-login Step 0, sap-doctor (`gui` group), sap-dev-init Step 1.1, sap-gui-probe Step 0 | Static, token-free session-status probe — emits `STATUS: NO_GUI\|NO_SCRIPTING\|NO_SESSION\|LOGIN_SCREEN\|LOGGED_IN` (+ detail lines). Run via 32-bit cscript. Tier-3 exempt (pre-login bootstrap). |
| `shared/scripts/sap_dpapi.ps1` | sap-login (Step 2a + 5b) | DPAPI encrypt/decrypt for `sap_password` at rest in `{work_dir}\runtime\connections.json` / `userconfig.json` (or a hand-edited dev-checkout `settings.local.json`) — never the tracked `settings.json`. Functions `Protect-SapSecret` / `Unprotect-SapSecret` (dot-source) + CLI mode (`-Action protect\|unprotect -Value <text>`). Encrypted values are stored as `dpapi:<base64>`; CurrentUser scope binds decryption to the Windows user account. Plaintext input passes through with a stderr warning for backward compatibility / migration. |
...
| `shared/scripts/sap_keyed_diff_lib.ps1` | **Keyed row-diff engine** — sap-config-compare, sap-se16n (snapshot diff) | The ONE keyed row-diff engine for the suite — pure-local (no SAP). `Get-SapKeyedDiff -LeftPath -RightPath -KeyColumns <cols> [-IgnoreColumns <volatile>]` joins two TSV extracts on the declared key and classifies every key ADDED / REMOVED / CHANGED / SAME, comparing only the columns COMMON to both sides minus key minus ignore — a row differing ONLY in an ignored/volatile column is SAME, not CHANGED; schema drift (columns on one side only) and duplicate keys are reported, never a silent pass; a missing key column throws (fail-loud). `Write-SapKeyedDiffTsv -Result -OutPath` writes an Excel-BOM `diff.tsv` (`row_class`/`key`/`changed_columns`/`detail`); `Get-SapKeyedDiffSummaryLine` → a one-line `KEYED_DIFF: added=… removed=… changed=… same=…` summary. Token `%%KEYED_DIFF_LIB_PS1%%`. |
...
| `shared/rules/safety_policy.md` | **ALL write-capable skills + /sap-login (mandatory — Rule 0; enforcement arm: `sap_safety_gate.ps1`, CI-enforced list in `check-consistency.mjs`)** | **The highest-priority rule set in this repository** (see Directive 0). Environment classification (`DEV`/`QAS`/`SBX`/`PRD`) on every connection profile at `/sap-login`; blank/unknown fails closed to `PRD`; `prod_write_policy` `BLOCK` (default, no override flag) / `TYPED_CONFIRM` (operator types `PROD <SID>/<CLIENT>` verbatim); a `SAFETY: REFUSED` verdict ends the run — never retried, softened, or hand-driven; policy changes only via `userconfig.json` outside the session. |
| `shared/rules/settings_lookup.md` | **ALL skills that read or write a userConfig value (mandatory — Rule 7)** | Four-tier model — env `SAPDEV_AI_WORK_DIR` (work_dir bootstrap only) > `settings.local.json` (gitignored dev-checkout override, hand-edited, never written by skills) > `{work_dir}\runtime\userconfig.json` (machine-global user overrides — the single skill WRITE target) > `settings.json` (tracked blank schema). Reads merge per-key on the `value` field. Path resolution from sap-dev-core skills vs. cross-plugin skills. Implementation paths for PowerShell (`sap_settings_lib.ps1`) and Claude-driven Read/Edit-tool flows. |
...
| `shared/rules/work_dir_onboarding.md` | /sap-login, /sap-dev-init (Step 0), `sap_workdir_setup.ps1` | The work_dir onboarding contract — probe / set / migrate flow, the `SAPDEV_AI_WORK_DIR` env var + `%APPDATA%\sapdev-ai\work_dir.txt` pointer pair, first-run signals, and the consent requirement before persisting either. |
...
| `shared/rules/sap_gui_security_handling.md` | Skills that trigger SAP GUI local-file IO (sap-se16n, sap-se38, sap-se37, sap-se24, sap-se11, sap-sp02, sap-atc, sap-trace) + `/sap-dev-init` Step 1b | Process contract for the SAP GUI Security dialog — why no VBS-side handler can exist (the Scripting COM API is fully suspended while the dialog is modal), the sidecar/warmup/grant/precheck division of labour (`sap_gui_security_sidecar.ps1` / `_warmup.vbs` / `_grant.ps1` / `_precheck.ps1`), the `saprules.xml` persistence model, and the coordination pattern (launch sidecar BEFORE the triggering action, `Wait-Process`, check the captured verdict for `FOUND_BUT_STUCK`). |
| `shared/rules/build_metrics.md` | `sap_build_kpi.ps1` + `/sap-log-analyze --builds` | Contract for the first-pass-yield build KPIs (schema `sapdev.buildkpi/1`) — clusters runs into logical builds from the two existing chokepoints (structured JSONL logs + the artifact index); cardinal rule: **derive, do not instrument** (no new per-build write path, no agent-prompt instrumentation — doc-instructed side effects have silently failed before, and STOP/partial builds are exactly what an append-a-row instruction misses). |
| `contributing/parallel_safe_session_attach.md` (repo-level, NOT shipped with the plugin) | **All authors writing new SAP-driving VBS templates (mandatory; enforced by CI gate in `sap-dev/scripts/check-consistency.mjs`)**. Read this BEFORE writing a new operational `.vbs` under `plugins/<plugin>/skills/<skill>/references/`. Lives outside the plugin because it has zero runtime callers — end users installing the plugin from marketplace don't need it. | Architectural contract for the **per-VBS attach pattern** (layer 1) that's complementary to the broker contract above; the doc now also summarizes the other three parallel-safety layers (layer 2 temp buckets, layer 3 per-session dev defaults, layer 4 per-session logs). Every SAP-driving VBS template must: (1) declare `Const SESSION_PATH = "%%SESSION_PATH%%"`, (2) `ExecuteGlobal`-include `%%ATTACH_LIB_VBS%%`, (3) call `Set oSession = AttachSapSession(SESSION_PATH)` — and nothing else for attach. Every wrapping SKILL.md PS block must substitute the two new tokens and set `$env:SAPDEV_SESSION_PATH` via `Get-SapCurrentSessionPath` (Phase 4.2). The rule doc spells out the canonical VBS pattern + the canonical SKILL.md wrapper convention as copy-paste blocks, lists the exempt files (the authoritative set is `TIER3_EXEMPT_VBS` in `sap-dev/scripts/check-consistency.mjs`) and why each is exempt, documents the four common gotchas (`Chr(37)` sentinel idiom, include order with session-lock, helper handles all error paths, `SAPDEV_SESSION_PATH` is read via env var), and ends with the CI verification command. **This contract's CI gate fires on twelve conditions** (authoritative spec: the per-gate comment blocks inside `check-consistency.mjs` — the checker has since grown further gates beyond this contract, e.g. Rule 0 safety-gate coverage) — five for session attach (legacy `For Each` idiom present, `SESSION_PATH` declared without `%%ATTACH_LIB_VBS%%` include, operational VBS with neither token (unmigrated), include without `AttachSapSession(...)` call (dead code), or wrapping SKILL.md missing the substitution when the template needs it) plus two for the run-temp model (a SKILL.md passing `{RUN_TEMP}` to `Get-SapCurrentSessionPath -WorkTemp` — hard error; a SKILL.md writing fixed-named generated scratch under the `{WORK_TEMP}` root instead of `{RUN_TEMP}` — warning) plus five later-added gates (missing referenced implementation files, bare `cscript`/`wscript` invocations, locale-literal GUI-text branching, non-ASCII bytes without a UTF-8 BOM, undocumented `shared/scripts` files). **History tail** at the bottom of the rule doc traces the chronology (3.0 helper → 3.1 read-only → 3.5 multi-conn → 3.2/3/4 bulk → 3.6 CI gate → 2026-06-20 layers 2–4: temp buckets / dev defaults / logs). Read this when joining the project or adding the first new SAP-driving skill. |
| `contributing/golden_screen_baselines.md` (repo-level, NOT shipped with the plugin) | **All authors adding/maintaining a SAP-driving VBS** — defines the screen-fingerprint baseline contract enforced by the coverage gate in `sap-dev/scripts/check-consistency.mjs`. Half 1 (static) of the GUI-robustness harness; half 2 is the `/sap-doctor --screens` live skill (PowerShell orchestrator `sap_screen_check.ps1` + self-resolving probe `sap_screen_check_probe.vbs`). | Per-VBS golden-screen baseline `references/<stem>.screens.json` (schema `sapdev.screenbaseline/1`): `{ schema, vbs, captured_on{release,kernel,date,method}, not_applicable_on[<release markers>], checkpoints[]{id, reach, identity{program,dynpro}, required_ids[<findById paths>], status, not_applicable_on[]} }`. **Two severities (2026-07-24, after the first cross-release sweep):** a missing required control ID = `DRIFT` / **BLOCKER** (the VBS will mis-step — fails the run); identity-only mismatch while every required control still resolves = `DRIFT_IDENTITY` / **WARN** (cross-release fingerprint difference, driver still works — non-blocking). `not_applicable_on` lists release markers where the screen doesn't exist at all (e.g. ATC and `/IWFND/ERROR_LOG` on `ECC6_EHP6`); those checkpoints are not probed and report `NOT_APPLICABLE`. Matched against the pinned profile's `server_release_marker` (or `-ReleaseMarker`); an unresolvable marker evaluates NO exemptions rather than risk a false PASS. Records the control IDs + screen identity each VBS depends on per checkpoint so drift (a release/locale that moved a control) is caught BEFORE a user hits a silent false-success. **CI gate** (driving VBS = declares `Const SESSION_PATH` / includes `%%ATTACH_LIB_VBS%%` / calls `AttachSapSession` / binds the Scripting engine, minus the Tier-3 exempt set): **HARD error** on a missing baseline (promoted from WARN 2026-07-24 when the ratchet reached 100% — a new driving VBS ships its static `pending_live` seed in the same commit) and on a malformed one; the `screen-baseline coverage N/M` figure stays in the checker summary. `status` ∈ `captured` (identity verified live) \| `pending_live` (static dependency-set seed, identity captured on first `/sap-doctor --screens` run). Coverage is **136/136** driving VBS (the last gap, `sap-stms`'s `sap_stms_import.vbs`, closed 2026-07-24 with a placeholder-free seed — its uncalibrated import-dialog controls are documented in the checkpoint note, not frozen as required IDs); most checkpoints remain `pending_live` (static dependency-set seeds), flipping to `captured` as `/sap-doctor --screens` verifies identity live per release. |
...
| `shared/rules/test_verdict_contract.md` | sap-test-replay, sap-golden-master, sap-run-abap-unit, /sap-evidence-pack (consumer) | **Test-verdict contract** (`sapdev.testverdict/1`) — one machine-readable TSV verdict line per test case (`case_id · producer_skill · verdict · evidence_path · run_id`); `FAIL` (real regression) kept strictly distinct from `REPLAY_ERROR` / `COULD_NOT_VERIFY` (test could not run) so flakiness never counts as a regression; producers write `verdict.tsv` into their artifact-dir run folder and register it (`kind=test-verdict-line`) for campaign-ledger / evidence-pack joins. |
...
| `shared/templates/customer_brief_sample.xlsx` | reference | Excel rendering of the filled-in sample customer brief, for customers who author the brief in Excel. EN only — `_JA` xlsx variant pending (see Template Language Resolution below). |
| `shared/templates/spec_template.xlsx` | sap-docs-extract input shape; sap-docs-layout `bootstrap` source | **Canonical design-spec workbook** — 17 content sheets (Cover, Interface Contract, Selection Screen, Selection Definition, Validation Rules, Processing Flow, Mapping (File In)/(File Out), Supplement, Domains, Data Elements, Tables, Error Messages, Text Elements, Golden Tests, Dependencies, README) + a `(Meta) Layout` sheet that maps each section to its output file (left visible in the committed templates; `/sap-docs-layout` hides it in production copies). Built by `tools/build_spec_template.py`. Customers copy this and fill in their project. |
