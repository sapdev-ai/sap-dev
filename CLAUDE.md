...
Version: 0.7.0 | Plugins: 4 | Last Updated: 2026-07-03
...
| **Group consistently inside the prefix.** Established sub-namespaces: `sap-docs-*` (spec pipeline), `sap-check-*` / `sap-fix-*` (validation pairs), `sap-rfc-wrapper-*` (codegen), `sap-gui-*` (GUI utilities), `sap-se##` / `sap-se##-*` (SAP transactions) | `sap-fix-abap`, `sap-rfc-wrapper-class` | `sap-fm-fix`, `sap-class-rfc-wrapper` |
