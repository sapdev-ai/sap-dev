...
| `shared/scripts/sap_attach_lib.vbs` | **All GUI-scripting VBS reference scripts that drive SAP GUI** — mandatory for skills being migrated under Tier 3 (parallel-safe session attach). Today's migrated set: `sap-se16n`, `sap-sp02`, `sap-where-used-list`, `sap-gui-diagnose` (Phase 3.1 read-only). | Shared session-attach primitive. Include via `ExecuteGlobal FSO.OpenTextFile("%%ATTACH_LIB_VBS%%",1).ReadAll()`. Exposes `Function AttachSapSession(sHint)` which returns a GuiSession bound to: (1) `sHint` if non-empty and resolves; (2) `SAPDEV_SESSION_PATH` env var if non-empty and resolves; (3) legacy first-session-of-first-connection. Replaces the ~25-line nested `For Each oCandidate In oApplication.Children` boilerplate that lived in every operational VBS. The convention: each migrated VBS declares `Const SESSION_PATH = "%%SESSION_PATH%%"`, includes this lib, and calls `Set oSession = AttachSapSession(SESSION_PATH)` — one line replaces the entire attach block. Calling skill wrappers (PowerShell) substitute `%%SESSION_PATH%%` with the parsed `--session` argument (or empty for legacy callers) and `%%ATTACH_LIB_VBS%%` with the absolute path to this file. Errors (SAP GUI not running, scripting engine unavailable, explicit hint that doesn't resolve, env-var path that doesn't resolve) are handled inside the helper — caller does not need its own error-handling block. The unsubstituted-token sentinel (used when the PowerShell wrapper hands through the literal `<%>%>SESSION_PATH<%>%>`) is detected via a `Chr(37)`-built runtime string so global wrapper substitution cannot corrupt it (same defence as `sap_gui_object_details.vbs`). Pairs with the broker (`sap_session_broker.ps1`): the broker decides WHICH session a task should use; this lib makes every VBS able to ATTACH to that decision. |
| `shared/scripts/sap_session_broker.ps1` | **All GUI-scripting skills that may run in parallel** (today: `/sap-gui-skill-scaffold` parallel path; will become broadly mandatory after Tier 3 migration). Full contract: `shared/rules/sap_session_broker.md`. | SAP GUI Session Broker. PowerShell, ~490 LOC. Single-binary CLI with five actions — `acquire` / `release` / `discover` / `gc` / `list` — driven by `-Action <name> -WorkTemp <abs-path>` + per-action args. State lives in `{WORK_TEMP}\session_registry.json` (UTF-8 no BOM); cross-process concurrency serialized by a named Windows mutex (`SapDevSessionBroker_v1`) acquired through `System.Threading.Mutex` with a 10s timeout for crash recovery. Reactive cleanup sweep runs inside every acquire/release, dropping entries on four failure modes — session closed (`findById` returns nothing), owner PID dead (`Get-Process` lookup; caller must pass `-OwnerPid $PID`), TTL expired, `SystemSessionId` changed (user logged out + back in). Idempotent on re-acquire by `task_id`. Spawns on demand via `/oSESSION_MANAGER` (the only OK-code mechanism verified on S/4HANA 1909 kernel 754; `CreateSession` and bare `/o` no-op). Stdout last line: `ACQUIRED: path=<p> sessionNumber=<n> reused=<bool>` / `RELEASED: path=<p>` / `NOT_FOUND` / `DENIED: <reason>` (exit 1) / `ERROR: <reason>` (exit 2). Shells out to `sap_session_broker_com.vbs` for every SAP-side operation because PowerShell 7+/.NET 5+ cannot bind the SAP GUI Scripting Engine directly (`Marshal::GetActiveObject` removed in .NET 5+; even 32-bit Windows PowerShell 5.1 fails to resolve the SAPGUI ProgID through the ROT). |
| `shared/scripts/sap_session_broker_com.vbs` | **Internal helper for `sap_session_broker.ps1`** — not intended for direct calls by other skills. | SAP COM helper for the broker. VBScript run via 32-bit `cscript`. Single argv command + JSON-on-stdout protocol. Commands: `INFO` (list all sessions of `/app/con[0]` with path / `SessionNumber` / `Transaction` / `ScreenNumber` / `has_popup`, plus the connection's `SystemSessionId` as `logon_id`), `SPAWN` (drive `/n` + `/oSESSION_MANAGER` on anchor session, return the newcomer's path + `SessionNumber`), `RESET <path>` (drive `/n` on a specific session — used by `release` to return the session to SAP Easy Access), `PROBE <path>` (single-session `findById` + `Info` read — used by acquire's pre-allocation Easy-Access verification). Exit codes: 0 success, 1 usage error, 2 SAP-unreachable, 3 command-level failure (details in JSON `error` field). JSON output is one line per invocation — broker parses with `ConvertFrom-Json`. |
...
| `shared/rules/sap_session_broker.md` | **All GUI-scripting skills that may run in parallel** — will become mandatory after the Tier 3 attach-helper migration. Today: `sap-gui-skill-scaffold` parallel path. | SAP GUI Session Broker contract — coordinates which AI task (sub-agent / skill run) drives which SAP GUI session via `acquire` / `release` / `discover` / `gc` / `list` CLI on `sap_session_broker.ps1`. Cross-process named-mutex serialises concurrent callers. Reactive cleanup sweep handles four failure modes — user closes a window, owner process crashes, TTL expires, user logs out and back in — surfaced as `DROP: <path> reason=<...>` lines. Identity caveat: SAP exposes no stable per-session ID on the kernels tested (`SystemSessionId` is per-logon; `(path, SessionNumber)` recycles), so the broker uses operational hygiene (Easy-Access verification on every acquire + `findById` re-resolution on every use) instead of identification. Pairs with `shared/scripts/sap_session_broker.ps1` (PowerShell broker) + `shared/scripts/sap_session_broker_com.vbs` (32-bit cscript COM helper — required because PowerShell 7+/.NET 5+ cannot bind to the SAP GUI Scripting Engine directly). |
