...
11. [Day-2 スキル: 実行、診断、修正、解説、移行](#11-day-2-スキル)
...
絞った編集 + 再デプロイで）、`GATE_VERDICT: PASS` になるまで再実行します。
...
レポートの実行はデプロイとは**異なるリスククラス**です — レポートはデータを変更しうるため、
`/sap-run-report` と `/sap-job schedule` は実行前に必ず明示的な確認のため停止します
（`cancel` / `delete` も同様。監視系の動詞は確認なしで実行されます）。バックグラウンド
実行は `Z_RUN_REPORT`（`/sap-dev-init` がデプロイ）経由のヘッドレスな RFC 高速パスを
優先し、ジョブのスプールを `/sap-sp02` で取得します。RFC が使えない場合は GUI での
スケジュールにフォールバックします。`/sap-file-transfer`（テキストまたはバイナリ、
上書きは明示的な `--overwrite` のみ）はリポジトリオブジェクトに一切触れません —
移送要求は関与しません — そしてファイルインターフェースのテストループを閉じます:
入力をアップロード → レポートを実行 → 出力をダウンロード → 仕様と突き合わせ。
...
- **取り返しのつかないすべてのアクションは確認する。** TR リリース、オブジェクト削除、
  レポート実行 / ジョブスケジュール（レポートは読み取り専用と**みなされません**）、
...
| `sap-run-report` | レポートをフォアグラウンド/バックグラウンドで実行（バリアント、アドホック値。実行前に必ず確認） |
| `sap-job` | バックグラウンドジョブのライフサイクル — schedule / list / status / log / spool / cancel / delete |
| `sap-file-transfer` | PC ↔ アプリケーションサーバーのファイル転送（CG3Z / CG3Y）+ RFC ディレクトリ一覧 |
