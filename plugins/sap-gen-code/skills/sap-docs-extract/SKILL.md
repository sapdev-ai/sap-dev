...
Read sap-dev-core's settings.json (resolve path: go 3 levels up from
`<SKILL_DIR>`, then into `sap-dev-core\settings.json`). Read `work_dir`,
`custom_url`, `design_docs_url`, `source_code_url`.
