...
| Comments language (§6) | `MODE_COMMENT_LANG` — default the **SAP logon language** of the active session; an explicit user instruction or a brief Comments-language value overrides. Rule §26. |
...
All generated **comments** (inline, headers, per-block) are written in
`MODE_COMMENT_LANG` — default the **SAP logon language** of the active
session; an explicit user instruction or a brief Comments-language value
overrides (rule §26). Distinct from selection-text / text-symbol output,
which follows the spec's natural language (§21).
...
Per rule §27, this applies to **optional** spec fields too — guard the
assignment with `IF … IS NOT INITIAL`, never skip it. A field parsed into
the staging structure but never assigned to any parameter (a "dead read")
is a silent data-loss defect, not an acceptable optimisation.
...
11. **All comments are in `MODE_COMMENT_LANG`** (default: the SAP logon
    language of the active session). Inline + header/banner + per-block
    comments match the resolved comment language; identifiers, keywords,
    and literals stay ASCII. (Rule §26 — developer-facing; distinct from
    the §21 selection-text / text-symbol output, which follows the spec's
    natural language.)
12. **No dead reads — every spec-mapped FM/method item is wired.** For each
    FM / BAPI / method call, every staging-struct field the spec maps to one
    of its parameters (file-map `SAP_TABLE`/`SAP_FIELD`, interface inputs,
    explicit params) has a corresponding assignment into the call —
    **including optional items** (guard with `IS NOT INITIAL`, never skip).
...
deploy syntax gate. Findings #11 (comment language) and #12 (dead reads)
are quality / data-loss defects that ATC may not flag — verify them by
reading the generated source before deploy.
