...
'   Change the package of a function module via SE37 > Goto > Object Directory.
'
...
' SE37: Goto > Object Directory Entry
Const MENU_OBJECT_DIR = "wnd[0]/mbar/menu[2]/menu[5]"
...
WScript.Echo "INFO: Session acquired. Changing FM " & sName & " package -> " & sPkg

' Navigate to SE37 + enter FM name
...
oSess.findById("wnd[0]/usr/ctxtRS38L-NAME").Text = sName
WScript.Sleep 200
If Err.Number <> 0 Then
    WScript.Echo "ERROR: FM name field not found (ctxtRS38L-NAME)."
    WScript.Quit 1
End If
Err.Clear
On Error GoTo 0
...
Sub ChangePackageFlow
    ...
    WScript.Sleep 200
    If Err.Number <> 0 Then
        WScript.Echo "ERROR: Package field not found: " & Err.Description
        WScript.Quit 1
    End If
    Err.Clear
