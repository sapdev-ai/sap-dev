...
'   Change the package of a message class via SE91 > Goto > Object Directory.
'
' Tokens: %%OBJECT_NAME%%, %%NEW_PACKAGE%%, %%TRANSPORT%%, %%TR_DESCRIPTION%%
' Output: INFO / STATUS_TYPE / STATUS_TEXT / DONE | ERROR
'
' SE91 needs an extra step: select "Message classes" radio + press Display
' (btnANZTASTE) so we land in the Display-MsgClass screen before opening
...
' SE91 Display screen: Goto > Object Directory Entry
...
WScript.Echo "INFO: Session acquired. Changing MSGCLASS " & sName & " package -> " & sPkg

' Navigate to SE91 + select Msg classes radio + Display
oSess.findById("wnd[0]").maximize
oSess.findById("wnd[0]/tbar[0]/okcd").Text = "/nse91"
oSess.findById("wnd[0]").sendVKey VKEY_ENTER
WScript.Sleep 1000

On Error Resume Next
oSess.findById("wnd[0]/usr/ctxtRSDAG-ARBGB").Text = sName
WScript.Sleep 200
If Err.Number <> 0 Then : WScript.Echo "ERROR: Msg class field not found (ctxtRSDAG-ARBGB)." : WScript.Quit 1 : End If
Err.Clear
' Select "Message Maintenance" radio (header tab MIDFLAG)
oSess.findById("wnd[0]/usr/radRSDAG-MIDFLAG").select
WScript.Sleep 200
Err.Clear
' Press Display (btnANZTASTE)
oSess.findById("wnd[0]/usr/btnANZTASTE").press
WScript.Sleep 1500
Err.Clear
On Error GoTo 0
