...
' Per language_independence_rules.md Rule 7: every multi-step write
' critical section (paste + save + activate) MUST wrap a session lock so
' the user can't steal focus mid-script. Helper degrades gracefully on
' older SAP GUI builds that don't expose LockSessionUI.
