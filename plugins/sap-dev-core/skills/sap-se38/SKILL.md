...
| `PASTE_VERIFY: FAILED after 3 attempts.` | Clipboard+SendKeys silently dropped on all 3 attempts. SAP GUI not in foreground at SendKeys time, or clipboard clobbered by another process. Editor still has the default 6-line SAP template; source did NOT land. **Pre-2026-05-27 this failure was silent** — Save / Syntax check / Activate all "succeeded" on the empty default, REPOSRC.DATALG ended up ~101 bytes vs the expected 1000s. Now caught by post-paste `getLineText()` verify. | Bring the SAP GUI window to the OS foreground (click on it once) and re-run `/sap-se38`. Do NOT touch keyboard/mouse during the retry — focus-stealing apps (Outlook toast, IM, browser tab) break the SendKeys path. ErrorClass=`SE38_PASTE_VERIFY_FAIL` |
...
$out    = '{WORK_TEMP}\THE_PROGRAM_NAME_from_sap.txt'   # the path SAP GUI will write
...
& 'C:/Windows/SysWOW64/cscript.exe' //NoLogo '{WORK_TEMP}\sap_se38_check_and_download_run.vbs'
# 4. Reap the watcher.
