...
' instead of the queue; without this guard the run would end in a misleading
' "queue-not-found / re-record IDs" abort (field 2026-07-11, S4H + ER1).
Dim sTmsAlert : sTmsAlert = DetectTmsAlert()
If Len(sTmsAlert) > 0 Then
    CloseTmsAlertPopup
    Fail "STMS_TMS_RFC_DOWN", "TMS communication layer is down (" & sTmsAlert & "). The import queue cannot render on this system; have Basis repair the TMSADM RFC destination, then re-run. Control-ID recording is NOT the fix here."
