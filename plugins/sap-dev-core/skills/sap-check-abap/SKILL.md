...
| `sap-dev-core/shared/templates/customer_brief.md` | *(config)* | Project Profile — used to set the quality bar (e.g. method length limit, modern-ABAP required, ATC priority gating). Resolved language-aware, first hit wins: `{custom_url}\customer_brief_<LANG>.md` → `{custom_url}\customer_brief.md` → shared `customer_brief_<LANG>.md` → this default (`<LANG>`: `userConfig.template_language`, else `userConfig.sap_language`, else `EN` — `EN` skips the `_<LANG>` probes). |
...
- **DDIC table types / domains / classes**: `DATA lt_x TYPE <ddic_table_type>.` — e.g. `TYPE filetable`, the standard table type used with `CL_GUI_FRONTEND_SERVICES=>gui_upload` — resolves correctly in online mode. The sidecar helper (`sap_rfc_lookup_ddic.ps1`) classifies the name via DD40L and returns kind `TTYP`; the checker treats it as a known type (no `TYPE_NOT_FOUND`) and reclassifies the variable to an internal table, so `lt_`/`gt_` is the expected naming prefix (not `lv_`). Domains (DD01L → `DOMAIN`) and global classes/interfaces (SEOCLASS → `CLASS`, via `TYPE REF TO`) resolve the same way. **Regression fixed 2026-07-03** (class 4 of the known false-positive set): the helper already queried DD40L, but the VBS consumer mapped only `STRUCT`/`DTEL` and folded every other resolved kind to `UNKNOWN`, producing a false `TYPE_NOT_FOUND` ERROR plus a false `NAMING` WARNING. Repro that must stay clean: `DATA lt_files TYPE filetable.`. Note: this resolution needs an SAP connection — in **offline mode** Phase 5b (type validity) is skipped entirely, so the false positive never manifested offline.
