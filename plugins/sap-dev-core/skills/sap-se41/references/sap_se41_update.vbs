' =============================================================================
' sap_se41_update.vbs  -  SAP GUI Scripting: Update existing PF-STATUS via SE41
'
' Generated by the sap-se41 skill.
' Attaches to an existing SAP GUI session, navigates to SE41, opens the status
' in change mode, reads function code definitions from a definition file,
' enters them into the editor field by field, saves, and activates.
'
...
' ------ 4. Enter program/status and press Change ----------------------------
...
oSession.findById("wnd[0]/usr/btn%#AUTOTEXT004").Press   ' Change button
WScript.Sleep 1000

' Verify we're in the editor
Dim sTitle
sTitle = oSession.findById("wnd[0]").Text
If InStr(sTitle, "Edit Status") = 0 Then
    WScript.Echo "ERROR: Could not open status for change. Title: " & sTitle
    WScript.Echo "       Sbar: " & oSession.findById("wnd[0]/sbar").Text
    WScript.Quit 1
End If
WScript.Echo "INFO: In editor: " & sTitle

' ------ 5. Navigate to Function Keys detail view ----------------------------
...
' Handle "Enter Function Text" popups
