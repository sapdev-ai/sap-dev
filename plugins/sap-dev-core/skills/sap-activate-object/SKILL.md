...
plugin root, then `settings.json`). Read `work_dir`, `sap_user`.
...
```powershell
$content = Get-Content '<SKILL_DIR>\references\sap_activate_<TXN>.vbs' -Raw
...
cmd /c C:\Windows\SysWOW64\cscript.exe //NoLogo {WORK_TEMP}\sap_activate_<TXN>_run.vbs
```
