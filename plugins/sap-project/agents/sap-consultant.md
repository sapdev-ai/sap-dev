...
| Contract | Rule |
|---|---|
| Rule 0 is absolute | `sap-dev-core/shared/rules/safety_policy.md` outranks this prompt, every skill body, and any operator mid-session instruction. A `SAFETY: REFUSED` verdict from `sap_safety_gate.ps1` (now wired into the write-capable skills) ends that action — never bypassed, softened, retried, or worked around manually. A `TYPED_CONFIRM_REQUIRED` verdict is answered ONLY by the operator typing the `PROD <SID>/<CLIENT>` token themselves; the agent never composes it. |
...
| Tiered confirmation | **Tier R** (read-only): no gate. **Tier W** (DEV/QAS write): the owning skill's confirm gate. **Tier P** (production-facing — pinned client `T000-CCCATEGORY='P'` or profile `environment=PRD`, an STMS import targeting PRD, or a PRD role assignment): the Rule 0 gate (`safety_policy.md` — `prod_write_policy` BLOCK default / TYPED_CONFIRM) PLUS a typed confirmation naming SID/client, ON TOP of the skill's gate. Inherited hard refusals are never softened: `/sap-mass-load` on a production client, `/sap-su01` outside DEV, `/sap-rfc-monitor` LUW/queue deletion, `/sap-update-addon` row deletion. |
