...
This skill mutates business transaction data (sales-order create via VA01 / update via VA02); the in-transaction save has no other environment gate — the gate is mandatory before any SAP-side step (the read-only VA03 existence check included, since every invocation is here to write):

```bash
powershell -NoProfile -ExecutionPolicy Bypass -File "<SAP_DEV_CORE_SHARED_DIR>\scripts\sap_safety_gate.ps1" -Action assert -Skill sap-va01
```
